...
void playlistwindow::on_pushButtonSavePlaylist_clicked()
{
    ...
    connect(playlistchoosename, SIGNAL(savePlaylist_(const QString &, int)),this,SLOT(savePlaylist(const QString &, int)));
...
void playlistwindow::savePlaylist(const QString &name, int val)
{
    mp->savePlaylist(name, val);
...
void playlistwindow::on_btnChangeListname_clicked()
{
    ...
    {
        QMessageBox::warning(this,"Playlist Yüklenmemiş","Playlist ismini değiştirebilmeniz için playlist yüklenmiş olmalıdır !");
        return;
    }
    playlistchoosename = new PlaylistChooseName(this, mp->isLoadedFromFile(), mp->getPlaylistTitle(),-1);
    connect(playlistchoosename, SIGNAL(savePlaylist_(const QString &, int)),this,SLOT(savePlaylist(const QString &, int)));
    playlistchoosename->show();
}
